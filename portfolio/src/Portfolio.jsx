...
// Center-relative offsets (px at scale 1). As a card flies toward the camera
...
// agentic AI: plan → act → observe loop around an LLM core,
// dashed arcs marching, tool call ticking underneath
function AgentGraph() {
  const ring = [
    { label: "plan", x: 110, y: 30 },
    { label: "act", x: 186, y: 138 },
    { label: "observe", x: 34, y: 138 },
  ];
  const arcs = [
    "M 124 40 Q 186 62 184 122",   // plan → act
    "M 170 148 Q 110 182 50 148",  // act → observe
    "M 36 122 Q 34 62 96 40",      // observe → plan
  ];
  return (
    <svg viewBox="0 0 220 200" className="viz">
      {arcs.map((d, i) => (
        <path key={d} d={d} className="agent-arc" style={{ animationDelay: `${i * 0.2}s` }} />
      ))}
      {ring.map((n) => (
        <g key={n.label}>
          <line x1="110" y1="96" x2={n.x} y2={n.y} className="agent-spoke" />
          <circle cx={n.x} cy={n.y} r="14" className="agent-node" />
          <text x={n.x} y={n.y + 3.5} textAnchor="middle" className="agent-lbl">{n.label}</text>
        </g>
      ))}
      <circle cx="110" cy="96" r="21" className="agent-core" />
      <text x="110" y="100" textAnchor="middle" className="agent-lbl agent-lbl-core">LLM</text>
      <text x="66" y="192" className="mono-tick">tool: retrieve(query)</text>
    </svg>
  );
}
...
function DomainReel({ reduced }) {
  ...
  const TOTAL = SCENES + 1; // + finale

  items.current = [];
  const register = (meta) => (el) => { if (el) items.current.push({ el, ...meta }); };

  // The camera flies forward through the scenes. Every element's transform is a
  // pure function of its signed distance d from the camera's focus plane:
  // d < 0 → still ahead in the distance (small, dim); d = 0 → at the viewer,
  // full size; d > 0 → already passed, rushing outward, blurring away.
  useEffect(() => {
    if (reduced) return;
    const wrap = wrapRef.current;
    if (!wrap) return;
    const clamp = (v, a, b) => Math.min(Math.max(v, a), b);
    // linear ramp: 0 at a, 1 at b
    ...
    // nearest one. Native scroll is never hijacked — any input cancels it. ---
    let idleT = 0;
    let snapRaf = 0;
    let snapping = false;
    ...
    const snapTo = (targetY) => {
      ...
      const dur = Math.min(850, 380 + Math.abs(dist) * 0.4);
    ...
    // any real input takes control back instantly mid-glide
    const onInput = () => {
      cancelSnap();
      clearTimeout(idleT);
      idleT = setTimeout(maybeSnap, 150);
  ...
  if (reduced) {
    return (
      <section className="section" id="domains">
        ...
        <h2 className="section-title">What I work in</h2>
        <div className="skills">
          {DOMAINS.map((d) => (
            <div key={d.word} className="panel skill-group">
              <h3 className="mono skill-title">{d.word}</h3>
              <p className="project-blurb">{d.caption}</p>
            </div>
          ))}
          <div className="panel skill-group">
            <h3 className="mono skill-title">Toolkit</h3>
            <ul className="chips">
              {chips.map((c) => (
  ...
  return (
    ...
    >
      <div className="reel-stage" ref={stageRef}>
        ...
        {DOMAINS.map((d, si) => (
          <div className="reel-scene" key={d.word}>
            <div className="reel-word-wrap" ref={register({ scene: si, kind: "word" })}>
            ...
            {d.cards.map((v, k) => {
              ...
              return (
                <div
                  ...
                  data-k={k}
                  className="reel-card"
                  ref={register({ scene: si, kind: "card", bx: slot.bx, by: slot.by, r: slot.r, k: 0.8 + k * 0.16 })}
                >
            ...
            {d.tags.map((t, ti) => (
              <span
                key={t}
                className={`mono reel-tag reel-tag-${ti}`}
                ref={register({ scene: si, kind: "tag" })}
              >
                {t}
              </span>
            ))}
        ...
        {/* finale: the five domain cards gather into a deck; toolkit + CTA */}
        <div className="reel-scene">
          {DOMAINS.map((d, di) => (
            <div
              key={di}
              className="reel-card deck"
              ref={register({ scene: SCENES, kind: "deck", di })}
            >
              <div className="reel-card-inner">
          ...
          <div className="reel-finale-copy" ref={register({ scene: SCENES, kind: "fin" })}>
            <span className="mono eyebrow">Domains</span>
  ...
  return (
    <div className={`root ${dark ? "theme-dark" : "theme-light"} ${parallaxOn ? "cursor-hidden" : ""}`} id="top">
      ...
      <main>
        ...
        {/* ---------------- domain reel (replaces Skills) ---------------- */}
        <DomainReel reduced={reduced} />
...
/* ---- domain reel (dolly-zoom fly-through) ----
   No sliding strip: every scene is a stack of planes at the vanishing point.
   JS drives each plane's scale/offset/blur from its distance to the camera,
   so scrolling reads as flying forward past the words and cards. */
.reel-wrap { position: relative; z-index: 1; /* height set inline */ }
.reel-stage {
  position: sticky; top: 0; height: 100svh; overflow: hidden;
}
.reel-scene { position: absolute; inset: 0; pointer-events: none; }
...
/* far/mid plane: the giant word, JS-driven */
.reel-word-wrap {
...
/* near plane: cards, JS-driven from center outward */
.reel-card {
...
/* HUD tags for density */
.reel-tag {
  position: absolute; z-index: 3; font-size: 10px; letter-spacing: 0.14em;
  color: var(--muted); opacity: 0; text-transform: uppercase; will-change: transform, opacity;
}
.reel-tag-0 { left: 9vw; top: 38vh; }
.reel-tag-1 { right: 9vw; top: 66vh; }
.reel-tag-2 { left: 47vw; top: 20vh; }
...
/* finale needs interactivity */
.reel-scene:last-of-type { pointer-events: none; }
.reel-finale-copy .chips, .reel-finale-copy .shiny-cta { pointer-events: inherit; }

@media (max-width: 767px) {
  .reel-word { font-size: 13vw; }
  .reel-card { width: 38vw; }
  .reel-card[data-k="2"] { display: none; }
  .reel-card.deck { width: 44vw; }
...
/* agent graph visual */
.agent-core { fill: color-mix(in srgb, var(--accent) 16%, transparent); stroke: var(--accent); stroke-width: 1.6; }
.agent-node { fill: color-mix(in srgb, var(--accent-2) 14%, transparent); stroke: var(--accent-2); stroke-width: 1.3; animation: nodePulse 2.8s ease-in-out infinite; }
.agent-spoke { stroke: var(--line); stroke-width: 1; }
.agent-arc {
  fill: none; stroke: var(--accent); stroke-width: 1.6; stroke-linecap: round;
  stroke-dasharray: 6 7; animation: march 1.1s linear infinite; opacity: 0.8;
}
@keyframes march { to { stroke-dashoffset: -26; } }
.agent-lbl { font-family: 'JetBrains Mono', monospace; font-size: 8.5px; fill: var(--fg); }
.agent-lbl-core { font-size: 9.5px; font-weight: 500; }
...
/* ---- skills (kept for the reduced-motion fallback of the reel) ---- */
